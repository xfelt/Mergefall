...
- **In-App Purchases:** In the **AppPlatformConfig** asset, set **Billing Product Premium Pack** and the **Coin Pack Product IDs** array (default: `gems_pouch`, `gems_stack`, `gems_chest`, `gems_vault`, `gems_hoard`, `starter_bundle`). In Google Play Console, create matching products (consumable for gem packs; `starter_bundle` is non-consumable). These IDs must also match the entries in the **CoinPackCatalog** (authorable under `Resources/MergeSurvivorData/CoinPackCatalog`, or the runtime fallback in `PrototypeBootstrap.Monetization.cs`). On a successful purchase, `IStoreService.PurchaseCompleted(productId)` fires and `StorefrontService.GrantPurchase` grants the Gems (+bonus% and any bundle hero). Do not commit real IDs if the repo is public; use a local override or CI.
- **Rewarded ads (placements):** The meta UI requests rewarded ads via `AdsService.ShowRewarded(placement, ...)` using `AdPlacements.{DoubleWinReward, FreeGems, FreeSpin, Revive, DailyDouble}`. All resolve to the single **Ad Unit Rewarded** ID; if you want per-placement reporting, create multiple AdMob units and map them. `free_gems`/`free_spin` are daily-capped client-side via `DailyRewardService`.
- **Gacha odds disclosure (required):** Google Play's loot-box policy requires disclosed odds. The Lucky Chest panel shows per-rarity probabilities from `GachaConfig`; keep that display in sync if you change the weights, and declare the mechanic in the Play Console listing.
